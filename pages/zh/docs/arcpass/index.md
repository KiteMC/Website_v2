...
- 修复 GUI 奖励编辑器修改金币后，界面显示新金额但保存和实际领取仍使用旧金额的问题。
- 同步修复名称、描述、物品数量、权限时长、称号优先级、粒子和样式等聊天输入字段的草稿保存。
- 修复 GUI 修改内置奖励后，重启时出现 `duplicate reward ID` 并导致插件初始化失败的问题。
- `custom.yml` 现在会作为最终自定义覆盖层加载，普通奖励文件之间的重复 ID 仍会被阻止。
