...
- 奖励可以使用任意指定物品作为 GUI 图标，并保留名称、描述、附魔和自定义数据。
- 奖励编辑 GUI 支持直接捕获玩家主手物品作为图标，也可一键恢复默认图标。
- GUI、消息和称号文本支持 `#RRGGBB` 与 `&#RRGGBB` 十六进制 RGB 色彩。
- 修复快速点击、重复命令或跨服操作可能导致奖励被重复领取的问题。
- 修复任务偶发重复完成、重复发奖，以及日常和周常任务在新周期无法再次完成的问题。
- 改善多通行证环境下的奖励领取判断，避免不同通行证之间互相影响。
