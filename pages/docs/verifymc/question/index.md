...
### 管理员如何登录？
管理员登录使用已注册玩家账号的密码，同时要求该玩家拥有服务器 OP 权限：

1. 先通过网页注册账号。
2. 在控制台或游戏内执行 `/op 玩家名`。
3. 点击网页右上角“登录”，输入注册时的用户名和密码。

VerifyMC 不提供单独的默认管理员密码。如果提示没有权限，请检查用户名是否与 `ops.json` 中的 OP 名称一致。
...
> 更多配置选项，请参阅 [配置文档](/docs/verifymc/file/config)。
